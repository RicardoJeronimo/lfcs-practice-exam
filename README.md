...
19. set the setuid bit of files in _./playground/filepool_ owned by **user3**. With `find ... \! ...` or `find ... -not ...`, list the files which do not have the setuid bit activated. Check their attributes with `lsattr` (`for file in `find ./playground/filepool/ -user user3 -not -perm /1000 -exec ls {} \+` ; do sudo lsattr $file ; done`).
