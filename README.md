...
38. create a systemd timer for __myservice__ (`/lib/systemd/system/myservice.timer`, see https://opensource.com/article/20/7/systemd-timers ). Add a cronjob to append the string '----------' in `/tmp/myservice.txt` every **10** seconds (`crontab -e`, and `*/10 * * * * echo ...`) 
