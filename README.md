...
38. create a systemd timer for __myservice__ (`/lib/systemd/system/myservice.timer`, see https://opensource.com/article/20/7/systemd-timers to run it every second). Add a cronjob to append the string '----------' in `/tmp/myservice.txt` every **10** seconds (`crontab -e`, and `*/10 * * * * echo ...`) 
